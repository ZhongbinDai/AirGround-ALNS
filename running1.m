function [bestIndividualSet, bestFitnessSet, bestIndividual, bestFitness] = running1(problemId, numOfDroneMaxDepart, populationSize, maxGeneration)
    % 从头开始跑
    [model] = initModel4(problemId, numOfDroneMaxDepart);
    ...
    for t = 1: maxGeneration
        ...
        [newPopulation] = perturbationOperation(population, model);             % 种群扰动
        newPopulation = repairOperation(newPopulation, model);          	    % 修复种群
	    ...
	    fprintf('算例%02d第%d代种群的最优值：%f\n', problemId, t, - bestFitness);
